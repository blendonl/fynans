"use client";

import Link from "next/link";
import { useMemo } from "react";
import { Clock, Plus, Loader2 } from "lucide-react";
import { usePendingExpenses } from "@/hooks/use-pending-transactions";
import { useFamilies } from "@/hooks/use-families";
import { useIntersectionObserver } from "@/hooks/use-intersection-observer";
import { PendingTransactionRow } from "./pending-transaction-row";

type TransactionStatus = "PENDING" | "REJECTED";

interface PendingTransactionListProps {
  status: TransactionStatus;
}

function PendingListSkeleton() {
  return (
    <div className="rounded-2xl bg-surface-variant/20 divide-y divide-border-light/50 overflow-hidden border border-glass-border-outer">
      {[0, 1, 2, 3].map((r) => (
        <div key={r} className="flex items-center gap-3 px-4 py-3.5">
          <div className="w-1 h-8 rounded-full bg-surface-variant skeleton-shimmer flex-shrink-0" />
          <div className="flex-1 space-y-1.5">
            <div className="h-3.5 w-24 rounded-md bg-surface-variant skeleton-shimmer" />
            <div className="h-3 w-32 rounded-md bg-surface-variant skeleton-shimmer" />
          </div>
          <div className="h-4 w-16 rounded-md bg-surface-variant skeleton-shimmer" />
        </div>
      ))}
    </div>
  );
}

export function PendingTransactionList({ status }: PendingTransactionListProps) {
  const { families } = useFamilies();
  const {
    data,
    isLoading,
    fetchNextPage,
    hasNextPage,
    isFetchingNextPage,
  } = usePendingExpenses(status, families);

  const allTransactions = useMemo(
    () => data?.pages.flatMap((page) => page.transactions) ?? [],
    [data],
  );

  const loadMoreRef = useIntersectionObserver(() => fetchNextPage(), {
    enabled: !!hasNextPage && !isFetchingNextPage,
  });

  if (isLoading) {
    return <PendingListSkeleton />;
  }

  if (allTransactions.length === 0) {
    const label = status === "PENDING" ? "pending" : "rejected";
    return (
      <div className="py-20 px-8 text-center">
        <div className="relative mx-auto mb-6 h-20 w-20 flex items-center justify-center">
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-warning/10 to-secondary/10" />
          <Clock className="h-10 w-10 text-text-disabled relative" />
        </div>
        <p className="text-base font-medium text-text-secondary">No {label} transactions</p>
        <p className="text-sm text-text-disabled mt-1 mb-6">
          {status === "PENDING"
            ? "Save transactions for review to see them here"
            : "Rejected transactions will appear here"}
        </p>
        <Link
          href="/add"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary text-white text-sm font-medium hover:bg-primary-variant transition-colors"
        >
          <Plus className="h-4 w-4" />
          Add Transaction
        </Link>
      </div>
    );
  }

  return (
    <div>
      <div className="rounded-2xl overflow-hidden border border-glass-border-outer bg-glass-bg divide-y divide-border-light/50">
        {allTransactions.map((transaction) => (
          <PendingTransactionRow
            key={transaction.id}
            transaction={transaction}
          />
        ))}
      </div>

      <div ref={loadMoreRef} className="h-1" />

      {isFetchingNextPage && (
        <div className="flex justify-center py-4">
          <Loader2 className="h-5 w-5 animate-spin text-text-secondary" />
        </div>
      )}
    </div>
  );
}
